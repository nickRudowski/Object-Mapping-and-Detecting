...
def detect_objects(object_images, scene_image_path, output_path, colors, obj_labels):
    ...
    for obj_idx, obj_path in enumerate(object_images):
        ...
        if len(good_matches) >= 10:
            ...
            obj_pts = np.float32(
                [obj_keypoints[m.queryIdx].pt for m in good_matches]
            ).reshape(-1, 1, 2)
            ...
            if M is not None:
                ...
                print(f"Failed to calculaate homography for {obj_path}.")
        else:
            res = cv2.matchTemplate(
                scene_gray, obj_gray, cv2.TM_CCOEFF_NORMED, mask=mask
            )
